...
# a[start]: 0..start 能偷到 max value: a[m-1]
...
# a[start]: 0..start 能偷到 max value: a[m]
...
# 设： 有 m 个房子，前 m 间能偷窃到的最高金额是 dp[m] ，前 m−1 间能偷窃到的最高金额是 dp[m−1] ，
# 此时向这些房子后加一间房，此房间价值为 num ；
# 加一间房间后： 由于不能抢相邻的房子，意味着抢第 m+1 间就不能抢第 m 间；
# 那么前 m+1 间房能偷取到的最高金额 dp[m+1] 一定是以下两种情况的 较大值 ：
# 不抢第 m+1 个房间，因此等于前 m 个房子的最高金额，即 dp[m+1] = dp[m]；
# 抢第 m+1 个房间，此时不能抢第 m 个房间；因此等于前 m−1 个房子的最高金额加上当前房间价值，
# 即 dp[m+1] = dp[m-1] + num；
# 细心的我们发现： 难道在前 m 间的最高金额 dp[m] 情况下，第 m 间一定被偷了吗？
# 假设没有被偷，那 m+1 间的最大值应该也可能是 dp[m+1]=dp[m]+num 吧？
# 其实这种假设的情况可以被省略，这是因为：
# 假设第 m 间没有被偷，那么此时 dp[m]=dp[m−1] ，此时 dp[m+1]=dp[m]+num=dp[m−1]+num ，
# 即可以将 两种情况合并为一种情况 考虑；
# 假设第 m 间被偷，那么此时 dp[m+1]=dp[m]+num 不可取 ，因为偷了第 m 间就不能偷第 m+1 间。
# 最终的转移方程： dp[m+1]=max(dp[m],dp[m−1]+num)
# 初始状态：
...
# 我们发现 dp[m] 只与 dp[m−1] 和 dp[m−2] 有关系，
