        ...
        for i in range(nb):
            ...
            for j in range(ns):
                ...
                if buttonsToAnyPos[i][sx][sy] != -1 and startToAnyPos[sx][
                    sy] != -1:
                    if tmp == -1 or tmp > buttonsToAnyPos[i][sx][sy] + \
                            startToAnyPos[sx][sy]:
            ...
            for j in range(i + 1, nb):
                ...
                for k in range(ns):
                    sx, sy = stones[k]
                    if buttonsToAnyPos[i][sx][sy] != -1 and \
                            buttonsToAnyPos[j][sx][sy] != -1:
                        if mn == -1 or mn > buttonsToAnyPos[i][sx][sy] + \
                                buttonsToAnyPos[j][sx][sy]:
                            mn = buttonsToAnyPos[i][sx][sy] + \
                                 buttonsToAnyPos[j][sx][sy]
                # 距离是无向图，对称的
                dist[i][j] = mn
                dist[j][i] = mn
            ...
            for i in range(nb):
                ...
                if mask & (1 << i) != 0:
                    for j in range(nb):
                        ...
                        if mask & (1 << j) == 0:
                            nextMask = mask | (1 << j)
                            if dp[nextMask][j] == -1 or dp[nextMask][j] > \
                                    dp[mask][i] + dist[i][j]:
