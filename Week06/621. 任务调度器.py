...
# 是否存在与任务A出现次数相同的任务。
# 该例子的计算过程为：
# (任务A出现的次数 - 1) * (m + 1) + (出现次数为3的任务个数），即：
# (3 - 1) * (2 + 1) + 2 = 8
# 所以整体的解题的步骤如下：
# 1. 计算每个任务出现的次数
# 2. 找出出现次数最多的任务，假设出现次数为 x
# 3. 计算至少需要的时间 (x-1) * (m+1)，记为min_time
# 4. 计算出现次数为 x 的任务总数 count_parts，计算最终结果为min_time + count_parts
# 特殊情况
# 然而存在一种特殊情况，例如：
# 输入：tasks = ["A", "A", "A", "B", "B", "B", "C", "C", "D", "D"], m=2
# 输出：10
...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        _max_count = max(counter.values())
        _num_max_count = tuple(counter.values()).count(_max_count)
        return max((_max_count - 1) * (n + 1) + _num_max_count, len(tasks))
