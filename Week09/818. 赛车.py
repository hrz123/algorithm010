...
# j < m
# 长度是n+j+1
# 还剩下的长度是t-2**(m-1)+2^j < t
# 所以递推方程
# f(i) = min(f(2**m-1-t) + m+ 1, f(t - (2**(m-1)-2^j)) + m+j+1), 0<=j < m
# 初始化
# f(0) = 0
# f(1) = 1
...
# 一种是走到 j = 2 ** (m-1) - 1, j < i
# 然后我们又往回走了 p = 2 ** k - 1, 现在我们的位置是 2 ** (m-1) - 2 ** k
# 指令长度是 A^(m-1)RA^kR m+k+1，我们现在距离终点 i - 2 **(m-1) + 2 ** k ,是小于i的
# 这样我们就得出了我们的递推公式
# f(i) = m if i == 2 ** m - 1
# f(i) = min(f(2**m -1 - i)+m+1, f(i-2**(m-1)+2**k)+m+k+1 k < m-1)
...
# f(i) = m if  == 2 ** m -1
# 1 i < 2**m-1
# A^nR指令长度n+1，距离终点f(2**m-1-i)
# 2 i > 2**(m-1)+1
# A^(m-1)RA^kR 指令长度n + k + 1
# 距离终点i - (2**(m-1)-1 - 2**k + 1)= i - 2**(m-1) + 2**k
# 两者之间求最小值
# 初始化f(0)= 0，其他求最小按说可以用正无穷初始化，但是因为我们会先赋值f(2**m-1-i) + m-1
# 所以初始化成什么无所谓
# 返回值
# f(target)
# 优化复杂度
...
# 往回走
# 2 ** k - 1
# 指令为 A^(m-1)RA^kR，距离终点为f(i - 2 ** (m-1)+2**k)
# 指令长度为n+k+1
# f(i) = m+k+1+f(i-2**(m-1)+2**k)
# k = 0..m-2
# 初始化和边界条件
# f(0) = 0
# 返回值f(target)
