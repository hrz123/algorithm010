...
# 线段树
# 我们定义一个操作get(a, m, r)表示查询a序列[m, r]区间内的最大子段和，
# 那么最终我们要求的答案就是get(arr, 0, m-1)。如何分治实现这个操作呢？
# 对于一个区间l, r，我们取m = (m + r)/2， 对区间[m,n] 和 [n+1, r]分治求解。
# 当递归逐层深入直到区间长度缩小为1的时候，递归「开始回升」。
# 这个时候我们考虑如何通过[m, n]区间的信息和[n+1, r]区间的信息合并成区间[m, r]的信息。
# 最关键的两个问题是：
# 我们要维护区间的哪些信息呢？
# 我们如何合并这些信息？
# 对于一个区间[m, r]，我们可以维护四个量：
# lSum表示[m, r]内以l为左端点的最大子段和
# rSum表示[m, r]内以r为右端点的最大字段和
# mSum表示[m, r]内的最大子段和
# iSum表示[m, r]的区间和
# 以下简称[m, n]为[m, r]的「左子区间」，[n+1, r]为[m, r]的右子区间。
# 我们考虑如何维护这些量呢（如何通过左右子区间的信息合并得到[m,r]的信息）？
# 对于长度为1的区间[start,start]，四个量的值都和ai相等。对于长度大于1的区间：
# 首先最好维护的是iSum，区间[m, r]的iSum就等于「左子区间」的iSum加上「右子区间」的iSum。
# 对于[m, r]的lSum，存在两种可能，它要么等于「左子区间」的lSum，
# 要么等于「左子区间」的iSum加上「右子区间」的lSum，两者取大。
# 对于[m, r]的rSum，同理，它要么等于「右子区间」的rSum，
# 要么等于「右子区间」的iSum加上「左子区间」的rSum，两者取大
# 当计算好上面的三个量之后，就很好计算[m, r]的mSum了。
# 我们可以考虑[m, r]的mSum对应的区间是否跨越m--它可能不跨越m，
# 也就是说[m, r]的mSum可能是「左子区间」的mSum和「右子区间」
# 的mSum中的一个；
# 它可能跨越m，可能是「左子区间」的rSum和「右子区间」的lSum求和。三者取大。
# 时间复杂度，相当于遍历一个二叉树的所有结点，结点最多为2*n个，所以时间为O(m)
# 空间复杂度：递归：O(logn)
class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        l, r = 0, len(nums) - 1
...
# 那么这种方法存在的意义是什么呢？
#
# 对于这道题而言，确实是如此的。但是仔细观察「方法二」，它不仅可以解决区间 [0,m−1]，
# 还可以用于解决任意的子区间 [m,r] 的问题。
# 如果我们把 [0,m−1] 分治下去出现的所有子区间的信息都用堆式存储的方式记忆化下来，
