...
# 什么样的两个字符可以构成回文串
# 假设存在两个字符串s1和s2，s1+s2是一个回文串，记这两个字符串的长度分别为len1和len2，
# 我们可以分三种情况进行讨论
# 1.len1==len2，那么s1是s2的反转
# 2.len(s1)>len(s2)，这种情况下我们可以将s1拆成左右两个部分：t1和t2，其中t1是s2的翻转，t2是回文串
# 3.len(s1)<len(s2)，这种情况下我们可以将s2拆成左右两个部分：t1和t2，其中t2是s1的翻转，t1是回文串
